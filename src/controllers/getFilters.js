...
const getFilterYear = async (req, res)=>{
    try {
        ...
        if(number){
            ...
            moviesYear.length ? res.status(200).json(moviesYear) : res.status(400).json("Year no Found")
...
const getFilterLanguage = async(req, res)=>{
    try {
        ...
        if(name){
            const movies = await Movie.findAll(); // db movies
            const moviesLanguage = movies.filter((movie)=> movie.language === name.toLowerCase())
        
            moviesLanguage.length ? res.status(200).json(moviesLanguage) : res.status(400).json("Language no Found")
        }else{
